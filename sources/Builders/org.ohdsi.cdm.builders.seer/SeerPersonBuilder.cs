...
namespace org.ohdsi.cdm.builders.seer
{
    /// <summary>
    ...
    public class SeerPersonBuilder : PersonBuilder
    {
        ...
        private bool died;
        private readonly HashSet<ProcedureOccurrence> procedureOccurrence = new HashSet<ProcedureOccurrence>();
        private readonly HashSet<Observation> observation = new HashSet<Observation>();
        private readonly HashSet<DeviceExposure> deviceExposure = new HashSet<DeviceExposure>();
        private readonly HashSet<ConditionOccurrence> conditionOccurrence = new HashSet<ConditionOccurrence>();
        private readonly HashSet<Measurement> measurement = new HashSet<Measurement>();
        ...
        public override Person BuildPerson(List<Person> records)
        {
            ...
            payerPlanPeriods = BuildPayerPlanPeriods(periods, person.PersonId);
            foreach (var pp in payerPlanPeriods)
            {
                pp.Id = chunkData.KeyMasterOffset.PayerPlanPeriodId;
            }
            ...
            foreach (var o in observations)
            {
                ...
                observationsRaw.Enqueue(o);
            }

            if (died)
                deathRecords.Enqueue(death);
        ...
        private void AddPerson(Person record, Person person)
        {
            switch (record.AdditionalFields["name"].ToUpper())
            {
                ...
                case "STATE":
                    ...
                    person.LocationId = vocabulary.LookupLocation(location) ?? 0;
                    break;
            }
        }
        ...
        public override void Build(Dictionary<string, long> providers)
        {
            ...
            if (person.YearOfBirth < 1900 || person.YearOfBirth > 2015) return;

            var observationPeriods = BuilObservationPeriods(payerPlanPeriods, person.PersonId).ToArray();
            if (observationPeriods.Any() &&
                observationPeriods.Any(period => person.YearOfBirth - period.StartDate.Year >= 2)) return;
            ...
            else if (payerPlanPeriods.Any())
            {
                var startDate = payerPlanPeriods.Min(op => op.StartDate);
                foreach (var o in observationsRaw)
                {
                    o.StartDate = startDate;
                }
            ...
            SetPayerPlanPeriodId(payerPlanPeriods.ToArray(), drugExposures, procedureOccurrences,
                visitOccurrences.Values.ToArray(),
                deviceExposures);

            // set corresponding ProviderIds
            SetProviderIds(drugExposures, providers);
            SetProviderIds(deviceExposures, providers);
            ...
            SetProviderIds(measurements, providers);
            ...
            AddToChunk(person, death, observationPeriods,
                ...
                procedureCosts, observations, measurements,
            ...
            DeviceExposure[] devExposure, DeviceCost[] deviceCost, Note[] notes)
        {
            ...
            foreach (var p in procedureOccurrence)
            {
                chunkData.AddData(p);
            }
            ...
            foreach (var c in conditionOccurrence)
            {
                chunkData.AddData(c);
            }
                ...
                switch (entityDomain)
                {
                    case "Condition":
                        ...
                        if (obs == null || obs.ValueAsNumber == 1)
                        {
                            ...
                            if (conditionOccurrence.Add(cond))
