...
namespace org.ohdsi.cdm.builders.optumintegrated
{
    ...
    public class OptumIntegratedPersonBuilder : PersonBuilder
    {
        ...
        private static void AddToDateClaimTypeDictionary(
            Dictionary<DateTime, Dictionary<string, List<IEntity>>> entities,
            ...
            Dictionary<long, VisitOccurrence> vo)
        {
            ...
            foreach (var similarDrugs in rxDrugs.SelectMany(drugs =>
                drugs.Value.GroupBy(d => d.SourceValue.Length >= 9 ? d.SourceValue.Substring(0, 9) : d.SourceValue)))
            {
                ...
                {
                    yield return similarDrugs.Where(d => d.SourceConceptId > 0).OrderBy(d => d.SourceValue.Length)
                        .Last();
                    continue;
                }
                ...
                dateClaimTypeDictionary)
        {
            ...
            {
                var prov =
                    dateClaimTypeDictionary[entity.StartDate][claimType]
                        .Where(c => c.VisitOccurrenceId == entity.VisitOccurrenceId)
                        .OrderBy(c => DateTime.Parse(c.AdditionalFields["visit_start_date"]))
                        .ThenBy(c => DateTime.Parse(c.AdditionalFields["visit_end_date"]))
                        .ThenBy(c => c.AdditionalFields["prov"])
            ...
            var cl2 = dateClaimTypeDictionary[entity.StartDate][claimType]
                .OrderBy(c => DateTime.Parse(c.AdditionalFields["visit_start_date"]))
                .ThenBy(c => DateTime.Parse(c.AdditionalFields["visit_end_date"]))
                .ThenBy(c => c.AdditionalFields["prov"])
                .ThenBy(c => c.AdditionalFields["provcat"])
            ...
            ObservationPeriod[] observationPeriods)
        {
            ...
            foreach (var de in BuildDrugExposuresOncology(oncologyRecords.ToArray(), visitOccurrences,
                observationPeriods))
            {
                yield return de;
            }
        ...
        private IEnumerable<ProcedureOccurrence> BuildProcedureOccurrencesSES(
                ...
                foreach (var sameVisit in sameStartDate.GroupBy(p => p.VisitOccurrenceId))
                {
                    foreach (var sameSource in sameVisit.GroupBy(p => p.SourceValue))
                    {
                        foreach (var sameConcept in sameSource.GroupBy(p => p.ConceptId))
                        {
                            ...
                            var procedureOccurrence = sameConcept.OrderBy(p => p.TypeConceptId)
                                .ThenBy(p => p.AdditionalFields["diag1"]).First();
            ...
            ObservationPeriod[] observationPeriods)
        {
            ...
            foreach (var r in base.BuildProcedureOccurrences(oncologyRecords.ToArray(), visitOccurrences,
                observationPeriods))
            {
                yield return r;
            }
        ...
        private IEnumerable<ConditionOccurrence> BuildConditionOccurrencesSES(
        ...
        private IEnumerable<Observation> BuildObservationsOncology(Observation[] observations,
            Dictionary<long, VisitOccurrence> visitOccurrences,
            ObservationPeriod[] observationPeriods)
        {
        ...
        private IEnumerable<Observation> BuildObservationsSES(Observation[] observations,
            Dictionary<long, VisitOccurrence> visitOccurrences,
            ObservationPeriod[] observationPeriods)
        {
            foreach (var observation in base.BuildObservations(observations, visitOccurrences, observationPeriods))
        ...
        private IEnumerable<Measurement> BuildMeasurementOncology(Measurement[] measurements,
            Dictionary<long, VisitOccurrence> visitOccurrences,
            ObservationPeriod[] observationPeriods)
        {
        ...
        private IEnumerable<Measurement> BuildMeasurementSES(Measurement[] measurements,
            Dictionary<long, VisitOccurrence> visitOccurrences,
            ObservationPeriod[] observationPeriods)
        {
        ...
        public IEnumerable<VisitOccurrence> GetIPClaims(VisitOccurrence[] visitOccurrences)
        {
            ...
            foreach (var ipGroup in visitOccurrences
                .Where(vo =>
                    vo.IdUndefined && vo.AdditionalFields != null && vo.AdditionalFields.ContainsKey("pat_planid"))
                .GroupBy(vo => vo.AdditionalFields["pat_planid"]))
            {
                var ipVisits = new List<VisitOccurrence>();
                foreach (var claim in ipGroup.Where(vo => vo.ConceptId == 9201).OrderBy(vo => vo.StartDate)
            ...
            ObservationPeriod[] observationPeriods)
        {
            ...
            foreach (var patplanidGroup in erVisits
                .Where(vo => vo.AdditionalFields != null && vo.AdditionalFields.ContainsKey("pat_planid"))
                .GroupBy(vo => vo.AdditionalFields["pat_planid"]))
            {
            ...
            foreach (var patplanidGroup in opVisits
                .Where(vo => vo.AdditionalFields != null && vo.AdditionalFields.ContainsKey("pat_planid"))
                .GroupBy(vo => vo.AdditionalFields["pat_planid"]))
            {
                foreach (var opGroup in patplanidGroup.GroupBy(v => v.StartDate))
                {
                    foreach (var opGroup1 in opGroup
                        .Where(vo => vo.AdditionalFields != null && vo.AdditionalFields.ContainsKey("prov"))
                        .GroupBy(v => v.AdditionalFields["prov"]))
                    {
            ...
            foreach (var entity in entities)
            {
                ...
                switch (entityDomain)
                {
                    ...
                    case "Meas Value":
                        ...
                        AddCost(m.Id, entity, CostV5ToV51_MesObser("Measurement"));
                        break;
                    ...
                    case "Procedure":
                        ...
                        AddCost(proc.Id, entity, CostV5ToV51_ProcDev("Procedure"));
                        break;
                    ...
                    case "Drug":
                        ...
                        {
                            AddCost(drg.Id, entity, v5 => new Cost
                            {
        ...
        public override IEnumerable<ProcedureCost> BuildProcedureCosts(ProcedureOccurrence[] procedureOccurrences)
        {
            ...
            foreach (var procedureOccurrence in procedureOccurrences)
            {
                if (procedureOccurrence.TypeConceptId != 38000254 &&
                    procedureOccurrence.TypeConceptId != 38000272) continue;
