...
namespace org.ohdsi.cdm.builders.truven_v5
{
    ...
    public class Truvenv5PersonBuilder : PersonBuilder
    {
        ...
        private readonly Dictionary<string, HashSet<VisitOccurrence>> caseidVisits =
            new Dictionary<string, HashSet<VisitOccurrence>>();

        private readonly Dictionary<string, HashSet<VisitOccurrence>> fachidVisits =
            new Dictionary<string, HashSet<VisitOccurrence>>();
        ...
        public override Person BuildPerson(List<Person> records)
        {
            ...
            if (records.Any(r => r.GenderConceptId != 8551 && r.GenderConceptId != person.GenderConceptId))
                return null; // Gender changed over different enrollment period 
        ...
        private VisitOccurrence GetVisitOccurrence(IEntity ent)
        {
            if (rawVisits.ContainsKey(ent.SourceRecordGuid))
            {
                ...
                if (vo.Id == 0 && rawVisits.ContainsKey(vo.SourceRecordGuid) &&
                    rawVisits[vo.SourceRecordGuid].SourceRecordGuid != ent.SourceRecordGuid)
                {
        ...
        private IEnumerable<T> JoinVisitOccurrences<T>(IEnumerable<T> entities) where T : class, IEntity
        {
            ...
            foreach (var entity in otherEntities)
            {
                ...
                else if (fachidKey != null)
                {
                    ...
                    var visitOccurrence = (fachidVisits[fachidKey].Where(fhV => fhV.ConceptId == 9201 && fhV.Id > 0)
                                               .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault() ??
                                           fachidVisits[fachidKey].Where(fhV => fhV.ConceptId == 9203 && fhV.Id > 0)
                                               .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault()) ??
                                          fachidVisits[fachidKey].Where(fhV => fhV.ConceptId == 9202 && fhV.Id > 0)
                                              .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault() ??
                                          fachidVisits[fachidKey].Where(fhV => fhV.ConceptId == 42898160 && fhV.Id > 0)
                                              .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault();

                    if (visitOccurrence == null)
                    {

                        visitOccurrence = (fachidVisits[fachidKey].Where(fhV => fhV.ConceptId == 9201)
                                               .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault() ??
                                           fachidVisits[fachidKey].Where(fhV => fhV.ConceptId == 9203)
                                               .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault()) ??
                                          fachidVisits[fachidKey].Where(fhV => fhV.ConceptId == 9202)
                                              .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault() ??
                                          fachidVisits[fachidKey].Where(fhV => fhV.ConceptId == 42898160)
                                              .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault();
                        ...
                        {
                            visitOccurrence = (fachidVisits[key].Where(fhV => fhV.ConceptId == 9201 && fhV.Id > 0)
                                                   .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault() ??
                                               fachidVisits[key].Where(fhV => fhV.ConceptId == 9203 && fhV.Id > 0)
                                                   .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate)
                                                   .FirstOrDefault()) ??
                                              fachidVisits[key].Where(fhV => fhV.ConceptId == 9202 && fhV.Id > 0)
                                                  .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault() ??
                                              fachidVisits[key].Where(fhV => fhV.ConceptId == 42898160 && fhV.Id > 0)
                                                  .OrderBy(v => v.StartDate).ThenBy(v => v.EndDate).FirstOrDefault();
                        }
        ...
        private static Func<ICostV5, Cost> CostV5ToV51(string domain)
        {
            return v5 => new Cost
            {
                ...
                RevenueCodeConceptId = v5.RevenueCodeConceptId ?? 0,
