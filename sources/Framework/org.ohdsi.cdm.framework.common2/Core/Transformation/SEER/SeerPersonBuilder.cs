...
namespace org.ohdsi.cdm.framework.common2.Core.Transformation.SEER
{
    /// <summary>
    ...
    public class SeerPersonBuilder : PersonBuilder
    {
        ...
        public override KeyValuePair<Person, Attrition> BuildPerson(List<Person> records)
        {
            ...
            {
                pp.Id = KeyMasterOffsetManager.GetKeyOffset(pp.PersonId).PayerPlanPeriodId;
            }

            foreach (var o in observations)
            {
                o.Id = KeyMasterOffsetManager.GetKeyOffset(records[0].PersonId).ObservationId;
        ...
        public List<ObservationPeriod> BuilObservationPeriods(List<PayerPlanPeriod> periods, long personId)
        {
            ...
            foreach (var ab in abEras.OrderBy(p => p.StartDate).ThenBy(p => p.EndDate))
            {
                ...
                {
                    Id = KeyMasterOffsetManager.GetKeyOffset(personId).ObservationPeriodId,
                    PersonId = personId,
                ...
                switch (entityDomain)
                {
                    ...
                    case "Procedure":
                    {
                        var po = entity as ProcedureOccurrence ??
                                 ...
                                 {
                                     Id =
                                         KeyMasterOffsetManager.GetKeyOffset(entity.PersonId)
