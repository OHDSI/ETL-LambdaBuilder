...
namespace org.ohdsi.cdm.framework.common.PregnancyAlgorithm
{
    public class PregnancyAlgorithm
    {
        ...
        {
            if (!validOutcomes.TryGetValue(outcomeType, out List<Event> value))
                return false;

            if (canBeEqual)
                return value.Any(pe => pe.Guid != e.Guid && pe.Date >= e.Date && pe.Date.Subtract(e.Date).TotalDays <= days);
            ...
            DrugExposure[] drugExposures)
        {
            ...
            {
                if (!outcomeLimits.TryGetValue(ol.FirstPregCategory, out Dictionary<string, OutcomeLimit> value))
                {
                    value = [];
                    outcomeLimits.Add(ol.FirstPregCategory, value);
                }

                value.Add(ol.OutcomePregCategory, ol);
            }
            ...
            if (validOutcomes.Count > 0)
            {
                var allEpisodes = FindPregnancEpisodes(validOutcomes, raw, termDurations, gestEsts,
                    ["OVUL", "OVUL2", "NULS", "AFP", "AMEN", "UP"], true, false).ToList();
                ...
                var pconf = FindPregnancEpisodes(validOutcomes, raw, termDurations, gestEsts,
                    ["PCONF", "AGP", "PCOMP", "TA"], false, false).ToArray();
        ...
        private static IEnumerable<ConditionEra> GetEpisodesAllStarts(Person person, List<KeyValuePair<Event, Event>> allEpisodes, List<KeyValuePair<Event, Event>> pconfEpisodes, KeyValuePair<Event, Event>[] contraEpisodes)
        {
            ...
            foreach (var episode in allEpisodes)
            {
                ...
                var e = new ConditionEra
                {
                    ...
                    OccurrenceCount = 0
                    //OccurrenceCount = Convert.ToInt32(episode.Value.ConceptId) // TMP
                };
            ...
            foreach (var group in all.GroupBy(e => e.SourceRecordGuid))
            {
                ...
                if (rank > 6)
                {
                    if (pconfEpisodes.Count > 0 || contraEpisodes.Length > 0)
                    {
                        ...
                        if (cc.Length == 0 && pc.Length != 0 && pc.Any(e => e.Value.Date < startDate))
                        {
                            startDate = pc.Where(e => e.Value.Date < startDate).OrderBy(e => e.Value.Date).First()
                                .Value.Date;
                            dateUpdated = true;
                        }
                        else if (pc.Length == 0 && cc.Length != 0 && cc.Any(e => e.Value.Date > startDate))
                        {
                            startDate = cc.Where(e => e.Value.Date > startDate).OrderBy(e => e.Value.Date).First()
                                .Value.Date;
                            dateUpdated = true;
                        }
                        else if (pc.Length != 0 && cc.Length != 0)
                        {
                            ...
                            if (!dateUpdated && afterCC.Length != 0 && pc.Any(e => e.Value.Date > startDate))
                            {
        ...
        private static Event FindStart(Event outcome, Event pe, TermDurations duration, DateTime priorOutcomeDate)
        {
            ...
            return episodeStartDate.Between(priorOutcomeDate > windowStart ? priorOutcomeDate : windowStart,
                windowEnd) ? new Event { EventId = pe.EventId, Category = pe.Category, Date = episodeStartDate, ConceptId = pe.ConceptId } : null;
        }
            ...
            bool hasDefault, bool desc)
        {
            ...
            foreach (var startMarker in markers)
            {
                ...
                foreach (var pe in raw.PregnancyEvents[startMarker].Where(e => e.Date <= outcome.Date)
                    .OrderBy(e => e.Date))
                {
            ...
            if (hasDefault)
            {
                    ...
                    wEnd))
                {
                    ...
                    else if (raw.PregnancyEvents.TryGetValue("FT", out List<Event> value) && value.Any(e =>
                                 e.Date.Between(priorOutcomeDate > wStart ? priorOutcomeDate : wStart,
                                     wEnd)))
                    {
