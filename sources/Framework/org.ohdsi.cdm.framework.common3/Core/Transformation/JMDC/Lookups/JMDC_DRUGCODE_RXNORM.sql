...
WHERE lower(SOURCE_VOCABULARY_ID) IN ('jmdc')
),
StandardJmdcDrugs as (
SELECT distinct SOURCE_CODE, TARGET_CONCEPT_ID, TARGET_DOMAIN_ID, SOURCE_VALID_START_DATE as VALID_START_DATE, SOURCE_VALID_END_DATE as VALID_END_DATE, SOURCE_VOCABULARY_ID, TARGET_VALUE_AS_CONCEPT_ID
FROM Source_to_Standard
...
,
Standard as ( 
select * from StandardJmdc
union 
select * from StandardJmdcDrugs where SOURCE_CODE not in (select SOURCE_CODE from StandardJmdc)
),
Source as (
...
order by S_S.SOURCE_CODE, Standard.SOURCE_VOCABULARY_ID
