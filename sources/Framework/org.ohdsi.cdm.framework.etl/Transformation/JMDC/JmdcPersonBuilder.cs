            ...
            foreach (var sameProviderId in conditions.GroupBy(c => c.ProviderId))
            {
                foreach (var sameSourceValue in sameProviderId.GroupBy(c => c.SourceValue))
                {
                    foreach (var episode in sameSourceValue.GroupBy(c => c.AdditionalFields["month_and_year_of_start"]))
                    {
                        ...
                        var startOfMedicalCare = DateTime.ParseExact(parentCondition.AdditionalFields["start_m_and_y_date"], "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
                        ...
                        if (diagnosis.Count == 1)
                        {
                            if (!_visitsDateDiagnosis.ContainsKey(diagnosis[0].VisitOccurrenceId.Value))
                            {
                                _visitsDateDiagnosis.Add(diagnosis[0].VisitOccurrenceId.Value,
                                    DateTime.ParseExact(diagnosis[0].AdditionalFields["start_m_and_y_date"], "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture));
                            }
                            else
                            {
                                var newValue = DateTime.ParseExact(diagnosis[0].AdditionalFields["start_m_and_y_date"], "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
