...
namespace org.ohdsi.cdm.framework.common.Helpers
{
    ...
    {
        public static IEnumerable<EraEntity> GetEras(IEnumerable<IEntity> entities, int gap1, long type1)
        {
            var filterdEntities = entities.Where(e => e.IncludeInEra()).ToList();
            var eraConceptIds = new HashSet<long>();
            ...
            foreach (var entity in filterdEntities)
            {
                ...
                {
                    entity.Ingredients = [entity.ConceptId];
                    eraConceptIds.Add(entity.ConceptId);
                }
            }

            foreach (int eraConceptId in eraConceptIds.Select(v => (int)v))
            {
                int ecId = eraConceptId;
                IEnumerable<IEntity> containedEntities = filterdEntities.Where(d => d.Ingredients.Contains(ecId));
                List<IEntity> ordered = [.. containedEntities.OrderBy(d => d.StartDate).ThenBy(d => d.EndDate)];
