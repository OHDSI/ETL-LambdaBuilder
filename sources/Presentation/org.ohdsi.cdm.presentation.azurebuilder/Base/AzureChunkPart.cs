...
{
    public class AzureChunkPart : IDisposable
    {
            ...
            foreach (var d in QueryDefinition.FindDefinition(definitions, reader))
            {
                ...
                else
                {
                    if (personIdsToSave.Contains(personId.Value))
                        ((AzureBlobReaderGzip)reader).Resume();
                    else
                    {
                        ((AzureBlobReaderGzip)reader).Pause();
                        return;
                    }
                }
                ...
                queryDefinition.ProcessedPersonIds[personId.Value] = ((AzureBlobReaderGzip)reader).RowIndex;
