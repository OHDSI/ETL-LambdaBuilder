...
namespace org.ohdsi.cdm.presentation.etl.Monitor
{
    ...
    //            var objects = new List<Tuple<string, DateTime>>();
    ...
    //        foreach (var o in CloadStorageHelper.GetObjectInfo(null, null, Settings.Current.CloudStorageName, prefix))
    //        {
    //            slices.Add(o.Item1.Split('.')[1]);
    //        }
    ...
    //            FirstOrDefault(sq => sq.Persons != null && QueryDefinition.IsSuitable(sq.Query.Database, Settings.Current.Building.Vendor)) ?? throw new Exception("Error: personDefinition == null");
    ...
    //    public IEnumerable<Tuple<string, DateTime>> GetObjects(string table, int chunkId, int slicesNum)
    //    {
    //        for (int i = 0; i < slicesNum; i++)
    //        {
    //            var prefix = $"{Settings.Current.BuildingPrefix}/{_cdmFolder}/{table}/{table}.{i}.{chunkId}.";

    //            foreach (var item in CloadStorageHelper.GetObjectInfo(null, null, Settings.Current.CloudStorageName, prefix))
    //            {
    //                yield return item;
    //            }
    //        }
    ...
    //    public Dictionary<long, bool> ProcessChunk(List<Tuple<string, DateTime>> objects, KeyValuePair<int, Dictionary<long, List<string>>> chunk, int slicesNum, bool onlyCheck)
    //    {
    //        var attempt = 0;
    //        var complete = false;
