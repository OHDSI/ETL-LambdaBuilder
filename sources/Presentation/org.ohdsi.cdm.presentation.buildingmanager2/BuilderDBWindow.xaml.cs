      ...
      private void ButtonOk_OnClick(object sender, RoutedEventArgs e)
      {
         try
         {
            ...
            using (var connection = SqlConnectionHelper.OpenMssqlConnection(sqlConnectionStringBuilder.ConnectionString))
            {
               ...
               using (var command = new SqlCommand($"ALTER DATABASE {builderDb} SET RECOVERY Simple", connection))
               {
                  command.ExecuteNonQuery();
               }
            }
            ...
            {
               foreach (var subQuery in buildingmanager2.Resources.Resources.BuilderDbScript.Split(new[] {"\r\nGO"}, StringSplitOptions.None))
               {
