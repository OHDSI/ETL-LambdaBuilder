#' @export
createCareSiteTests <- function (frameworkType) {

  patient <- createPatient()
  encounter <- createEncounter()  
  declareTest(id = patient$person_id, "CareSite. Patient condition_concept_id of 0, no condition_era record created. Id is PERSON_ID.")
  ...
  patient <- createPatient()
  encounter <- createEncounter()  
  declareTest(id = patient$person_id, "CareSite. Two records with same condition_concept_id are less than 30 days apart, one condition_era record created. Id is PERSON_ID.")
  add_enrollment_detail(enrolid=patient$enrolid, dtend = '2012-12-31', dtstart = '2012-01-01')
  ...
  patient <- createPatient()
  encounter <- createEncounter()  
  declareTest(id = patient$person_id, "CareSite. Two records with same condition_concept_id are more than 30 days apart, two condition_era records created. Id is PERSON_ID.")
  ...
  patient <- createPatient()
  encounter <- createEncounter()  
  declareTest(id = patient$person_id, "CareSite. PLACE_OF_SERVICE_CONCEPT_ID = 0")
  add_enrollment_detail(enrolid=patient$enrolid, dtend = '2013-12-31', dtstart = '2012-01-01')
  add_outpatient_services(enrolid=patient$enrolid, dx1='37613', dxver = '9', svcdate = '2012-07-01')
  expect_care_site(place_of_service_concept_id = 0)
}