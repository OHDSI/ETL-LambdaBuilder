createObservationPeriodTests <- function () {
  ...
  patient<-createPatient();
  declareTest(id = patient$person_id, "When a person has a death recorded then we truncate the OBSERVATION_PERIOD_END_DATE to reflect the corresponding DEATH_DATE. Id is PERSON_ID.")
  add_enrollment_detail(enrolid=patient$enrolid, dtstart="2012-01-01", dtend="2012-12-31")
  add_facility_header(enrolid=patient$enrolid, svcdate="2012-08-02", dstatus="20")
  expect_observation_period(person_id=patient$person_id, observation_period_start_date="2012-01-01", observation_period_end_date="2012-08-02")
  
  patient<-createPatient();
  declareTest(id = patient$person_id, "When a person has a death recorded then we truncate the OBSERVATION_PERIOD_END_DATE to reflect the corresponding DEATH_DATE. Id is PERSON_ID.")
  add_enrollment_detail(enrolid=patient$enrolid, dtstart="2012-01-01", dtend="2012-12-31")
  add_inpatient_services(enrolid=patient$enrolid, svcdate="2012-09-02", dstatus="20")
  expect_observation_period(person_id=patient$person_id, observation_period_start_date="2012-01-01", observation_period_end_date="2012-09-02")
