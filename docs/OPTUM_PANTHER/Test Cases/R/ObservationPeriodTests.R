createObservationPeriodTests <- function () {
  ...
  patient <- createPatient();
  declareTest("Tests if 2 patient durations (where second is within 30 days of first) results in a single observation period", id = patient$person_id)
  ...
  patient <- createPatient();
  declareTest("Tests if 2 patient durations (where second starts 33 days after first) results in two observation periods", id = patient$person_id)
  ...
  patient <- createPatient();
  declareTest("Patient duration is 1 month", id = patient$person_id)
