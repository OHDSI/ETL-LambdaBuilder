initFramework <- function() {
  ...
  defaults <- list()
  defaults$patid <- '802666599315676'
  defaults$pat_planid <- '208426552558258'
  defaults$clmid <- ''
  defaults$icd_flag <- '9'
  defaults$proc <- '9999'
  ...
  defaults$episode_id <- ''
  defaults$patid <- ''
  defaults$dt_start <- '2021-03-01'
  defaults$dt_end <- '2023-06-30'
  defaults$visit_type <- 'ER'
  assign('pos_episode_visit', defaults, envir = frameworkContext$defaultValues)
  
  defaults <- list()
  defaults$patid <- '802666553891031'
  defaults$pat_planid <- '208426580476587'
  defaults$abnl_cd <- ''
  defaults$anlytseq <- '01'
  defaults$fst_dt <- '2021-02-10'
  ...
  defaults$prov <- '9555052297'
  defaults$prov_par <- 'P'
  defaults$provcat <- '0001'
  defaults$refer_prov <- '0'
  defaults$rvnu_cd <- ''
  defaults$service_prov <- '5555555559'
  defaults$std_cost <- '0.00'
  defaults$std_cost_yr <- '2019'
  ...
  defaults <- list()
  defaults$prov_unique <- '1802655556559886'
  defaults$dea <- ''
  defaults$npi <- ''
  defaults$prov <- ''
  defaults$extract_ym <- '202309'
  ...
  defaults$d_household_income_range_code <- '6'
...
set_defaults_member_enrollment <- function(patid, pat_planid, aso, bus, cdhp, eligeff, eligend, gdr_cd, group_nbr, health_exch, lis_dual, product, yrdob, extract_ym, version, family_id, ethnicity, race, race_source, region) {
...
set_defaults_member_continuous_enrollment <- function(patid, eligeff, eligend, gdr_cd, yrdob, extract_ym, version, ethnicity, race, race_source) {
...
set_defaults_provider <- function(prov_unique, bed_sz_range, cred_type, grp_practice, hosp_affil, prov_type, provcat, taxonomy1, taxonomy2, extract_ym, version, prov_region) {
...
set_defaults_ses <- function(patid, d_education_level_code, d_home_ownership_code, d_household_income_range_code, d_networth_range_code, extract_ym, version) {
...
add_member_enrollment <- function(patid, pat_planid, aso, bus, cdhp, eligeff, eligend, gdr_cd, group_nbr, health_exch, lis_dual, product, yrdob, extract_ym, version, family_id, ethnicity, race, race_source, region) {
  defaults <- get('member_enrollment', envir = frameworkContext$defaultValues)
  fields <- c()
  values <- c()
  if (missing(patid)) {
  ...
  if (missing(region)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'member_enrollment.region')
  }
...
add_member_continuous_enrollment <- function(patid, eligeff, eligend, gdr_cd, yrdob, extract_ym, version, ethnicity, race, race_source) {
  defaults <- get('member_continuous_enrollment', envir = frameworkContext$defaultValues)
  fields <- c()
  values <- c()
  if (missing(patid)) {
...
add_pos_episode_visit <- function(episode_id, patid, dt_start, dt_end, visit_type) {
  ...
  if (missing(patid)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'pos_episode_visit.patid')
  }
  fields <- c(fields, "patid")
  values <- c(values, if (is.null(patid)) "NULL" else if (is(patid, "subQuery")) paste0("(", as.character(patid), ")") else paste0("'", as.character(patid), "'"))
...
add_provider <- function(prov_unique, bed_sz_range, cred_type, grp_practice, hosp_affil, prov_type, provcat, taxonomy1, taxonomy2, extract_ym, version, prov_region) {
  defaults <- get('provider', envir = frameworkContext$defaultValues)
  fields <- c()
  values <- c()
  if (missing(prov_unique)) {
  ...
  if (missing(prov_region)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'provider.prov_region')
  }
...
add_ses <- function(patid, d_education_level_code, d_home_ownership_code, d_household_income_range_code, d_networth_range_code, extract_ym, version) {
  defaults <- get('ses', envir = frameworkContext$defaultValues)
  fields <- c()
  values <- c()
  if (missing(patid)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'ses.patid')
  }
  fields <- c(fields, "patid")
  values <- c(values, if (is.null(patid)) "NULL" else if (is(patid, "subQuery")) paste0("(", as.character(patid), ")") else paste0("'", as.character(patid), "'"))
  ...
  if (missing(d_education_level_code)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'ses.d_education_level_code')
  }
  ...
  if (missing(d_home_ownership_code)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'ses.d_home_ownership_code')
  }
  ...
  if (missing(d_household_income_range_code)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'ses.d_household_income_range_code')
  }
  ...
  if (missing(d_networth_range_code)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'ses.d_networth_range_code')
  }
  ...
  if (missing(extract_ym)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'ses.extract_ym')
  }
  fields <- c(fields, "extract_ym")
  values <- c(values, if (is.null(extract_ym)) "NULL" else if (is(extract_ym, "subQuery")) paste0("(", as.character(extract_ym), ")") else paste0("'", as.character(extract_ym), "'"))
  ...
  if (missing(version)) {
    ...
    frameworkContext$sourceFieldsTested <- c(frameworkContext$sourceFieldsTested, 'ses.version')
  }
  fields <- c(fields, "version")
  values <- c(values, if (is.null(version)) "NULL" else if (is(version, "subQuery")) paste0("(", as.character(version), ")") else paste0("'", as.character(version), "'"))
